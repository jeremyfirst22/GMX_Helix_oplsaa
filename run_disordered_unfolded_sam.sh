...
main(){
    ...
    if [ ! -d disordered_$SOL ] ; then mkdir disordered_$SOL ; fi 
    cd disordered_$SOL
    if [ $fold == "prep" ] ; then 
        prep
    else  
...
checkInput(){
    ...
    MOLEC=folded_$SOL
    logFile=$TOP/disordered_$SOL/$fold/$fold.log
    errFile=$TOP/disordered_$SOL/$fold/$fold.err
    if $verbose ; then 
        echo "MOLEC : $MOLEC"
        echo "SOL : $SOL"
    ...
    if [ ! -z $fold ] ; then 
        if [[ $fold != 'unfolded' ]] ; then #&& $fold != 'unfolded' && $fold != 'prep' ]] ; then 
            echo "ERROR: This script is only for running the unfolded DISORDERED SAM"
