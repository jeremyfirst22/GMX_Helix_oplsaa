...
HELP(){
    echo 
    echo "This programs is a wrapper for the run_single_bound_sam.sh, and run_not_bound_sam.sh scripts." 
    echo 
...
solventList="single_bound_sam not_bound_sam" 
molList="folded unfolded" 

if $prep ; then 
    for solvent in $solventList ; do 
        if [[ "$solvent" = "single_bound_sam" ]] ; then 
            sol="wat"
