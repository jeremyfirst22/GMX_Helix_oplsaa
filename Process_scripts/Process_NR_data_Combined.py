...
sampleNames = {
        ...
        "5" : {"D2O": 229453, "Air": 229449, "7030": 390033}, 
...
bottom, top = 0.08, 0.95
hspace, wspace = 0.25, 0.15

fig,axarr = plt.subplots(2,2,figsize=(6.50,4))
...
fig.text(left+(right-left-wspace/2)/4 , 0.49, "$Q$ ($\AA^{-1}$)", ha='center', va='bottom')
fig.text(right-(right-left-wspace/2)/4, 0.49, "$Q$ ($\AA^{-1}$)", ha='center', va='bottom')
fig.text(left+(right-left-wspace/2)/4, 0.00, "Distance from Si interface ($\AA$)", ha='center', va='bottom')
fig.text(right-(right-left-wspace/2)/4, 0.00, "Distance from Si interface ($\AA$)", ha='center', va='bottom')

fig.text(left+(right-left-wspace/2)/4, top+0.01, r"\textbf{Replicate 2}", ha='center', va='bottom') 
fig.text(right-(right-left-wspace/2)/4,top+0.01, r"\textbf{Replicate 3}", ha='center', va='bottom') 

axarr[0,0].text(0.05,0.95,r"\textsf{A}",va='top',ha='left',transform=axarr[0,0].transAxes,fontsize=12)
axarr[1,0].text(0.05,0.95,r"\textsf{B}",va='top',ha='left',transform=axarr[1,0].transAxes,fontsize=12)
axarr[0,1].text(0.05,0.95,r"\textsf{C}",va='top',ha='left',transform=axarr[0,1].transAxes,fontsize=12)
axarr[1,1].text(0.05,0.95,r"\textsf{D}",va='top',ha='left',transform=axarr[1,1].transAxes,fontsize=12)

sampleList = ["5", "2" ]
for col,sample in enumerate(sampleList) : 
    ax1, ax2 = axarr[:,col]
    ###
...
#    ax1.set_yticks(np.geomspace(10**-9, 10**4, num=16))  
    ...
    for env in envList : 
        ...
        try : 
            ...
            print "Skipping SLD %s %s"%(env, sample) 
            continue 
        index += 1
...
fig.legend(loc='upper right', bbox_to_anchor=(left+(right-left-wspace/2)/2,top)) 

fig.savefig('figures/combined_SI_NR_data.png',format='png')
